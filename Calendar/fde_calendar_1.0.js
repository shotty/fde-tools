...
function Calendar(parameters){
	...
	this.onMonthLoad = eval(this.APPLICATION_PARAMETERS['onMonthLoad']);
	this.onSelection = eval(this.APPLICATION_PARAMETERS['onSelection']);
	this.onErrorDate = eval(this.APPLICATION_PARAMETERS['onErrorDate']);
	
	
	this.buildCalendar = !(this.APPLICATION_PARAMETERS['buildCalendar'] == 'false')
	...
	if (this.wrapper) {
		...
		/* TODO: set the right HTML into opener link */
		this.opener.innerHTML = 'v';
		
		this.opener.onclick = Calendar._bindEvent(this.open, this, [null, 0]);
...
Calendar._utility = {
	...
	'getDateObjectByFormat':function( date , format ){
		...
		var iDD = format.indexOf('DD');
		var iMM = format.indexOf('MM');
		var iYY = ( this.isShortYear ? format.indexOf('YY')  :  format.indexOf('YYYY') );
		
		obj.DD = (iDD!=-1) ? parseInt( date.substring( iDD , iDD + 2 )  , 10  ) : 0;
		obj.MM = parseInt( date.substring( iMM , iMM+2 )  , 10  );
		
		if ( this.isShortYear ){
			var _Year = date.substring( iYY , iYY+2 );
			_Year = parseInt( _Year , 10 );
			if ( _Year < Calendar._SHORT_YEAR_LIMIT )
				obj.YYYY = parseInt( '20' + ( (_Year<10) ? '0' + _Year : _Year ) , 10 );
			else
				obj.YYYY = parseInt( '19' + ( (_Year<10) ? '0' + _Year : _Year ) , 10 );
		}else{
			obj.YYYY = parseInt (   date.substring( iYY , iYY+4 )  , 10  );
		}
		return obj;
	},
	...
	'getDateFromString':function( date ){
		if ( !date || date.length==0)
			return null;
		var obj = this.getDateObjectByFormat( date );
		var _date = new Date();
		_date.setMonth( obj.MM -1 );
		_date.setDate( (obj.DD > 0) ? obj.DD : 1 );
		_date.setYear( obj.YYYY );
	...
	'buildCorrectDate':function( date ){
		var val1 = Calendar._occurrencesOf(date, this.separator);
		var val2 = Calendar._occurrencesOf(this.format, this.separator);
		
		if ( val1 == 0 || ( date.length != this.format.length)  ) {
			...
			var iDD = _Format.indexOf( 'DD' );
			var iMM = _Format.indexOf('MM');
			var iYY = ( this.isShortYear ) ?  _Format.indexOf('YY') : _Format.indexOf('YYYY') ;
			
			var obj = {
				'DD': date.substring( iDD ,  iDD + 2 ),
				'MM': date.substring( iMM ,  iMM + 2 ),
				'YYYY': date.substring( iYY ,  iYY + ( ( this.isShortYear || _shortDate ) ? 2 : 4 ) )
			};
			
			if ( _shortDate )
				obj.YYYY = (  parseInt( obj.YYYY , 10 ) < Calendar._SHORT_YEAR_LIMIT   )  ?  '20' + obj.YYYY : '19' + obj.YYYY;
						
			_Format = this.format;
			_Format = _Format.replace('DD' , obj.DD );
			_Format = _Format.replace('MM' , obj.MM );
			_Format = _Format.replace( ( ( this.isShortYear) ? 'YY' : 'YYYY' ) , obj.YYYY );
...
Calendar._architecture = {
	 ...
	 */
	'open':function( event , position , flag ){
	...
	'mouseWheel':function( event , option ){
		...
		/*this.lockScroll = ( this.lockScroll ) ? this.lockScroll+1 : 1;
		if ( this.lockScroll == 2 ){
			Calendar._stopEvent( event );
...
Calendar._core = {
	...
	'load':function( flag ){
		...
		if ( this.text ) this.selectedDate = this.getDateFromString(  this.text.value );
		...
		var day = 1;
		_Date.setDate( day );
		var indx = _Date.getUTCDay();
		if ( indx == 0)
			indx =7;
		while(  true  ){
			
			this.tdDays[ indx ].innerHTML = _Date.getDate();
			
			if ( this.selectedDate  &&   _Date.getTime() == this.selectedDate.getTime() )
				Calendar._insertClass( this.tdDays[ indx ] , Calendar._css['selected']  );
			
			var disabled =  ( !this.isBetweenDate(_Date, this.minDate, this.maxDate)  ) || 
							(  ( typeof this.onMonthLoad == 'function' ) &&  ! this.onMonthLoad(day, MONTH, YEAR, this)  );
			
			if (!disabled) {
				this.tdDays[indx].onclick = Calendar._bindEvent(this.onDateSelection, this, [Calendar._copyDate(_Date)]);
				Calendar._insertClass( this.tdDays[ indx ] , Calendar._css['navigation']  );
			}else 
				Calendar._insertClass(this.tdDays[indx], Calendar._css['disabled']);
		...
		_Date.setYear( YEAR );
		this.currentDate = _Date; //Calendar._copyDate( _Date );
		this.spanMonth.innerHTML = this.MONTHS[ MONTH ];
		this.spanYear.innerHTML = YEAR
...
Calendar._load=function( ev ){
	...
	var calendars = document.getElementsByTagName( Calendar._TAG );
	for ( var i =(calendars.length-1) ; i>=0; i--  ){
		var calendar = calendars[i];
		var cssClasses = calendar.className.split(' ');
		if (  Calendar._contains (   cssClasses , Calendar._css['calendar_element']  )   ){
			var params = {};
			if ( Calendar._ie && Calendar._TAG == Calendar._TAGS[0] ){ //Check if special tag
				var _Calendar = calendar.nextSibling;
				while( _Calendar.tagName != '/' + calendar.tagName ){	
					if (  _Calendar.tagName.toLowerCase() == Calendar._OPTION_TAG.toLowerCase() ){
						params[ _Calendar.getAttribute('name')  ] = _Calendar.getAttribute('value');
					}
					_Calendar = _Calendar.nextSibling
				}
			}else{

				calendar.getOptions = calendar.getElementsByTagName;
				var options = calendar.getOptions( Calendar._OPTION_TAG );
				for ( var j=0; j<options.length;j++){
					params[ options[j].getAttribute('name')  ]   =   options[j].getAttribute('value');
				}
			}
			params['node'] = calendar;
...
/*Action parameters*/
Calendar._applParams = ['format','min','max','start','disabled','draggable','language','node','onMonthLoad','onSelection','onErrorDate', 'buildCalendar'];
